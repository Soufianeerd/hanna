...
async function runFullFlowVerification() {
  ...
  try {
    ...
    // Attente de l'entrée et stabilisation en idle (~3.4s)
    await sleep(3400);
    ...
    // 03_flip_90.png (stabilisation 220ms + pause 80ms + mi-flip 1.55s / 2 = ~775ms -> total ~1075ms)
    await sleep(1075);
    console.log('5. Capturing 03_flip_90.png (tranche 90° ralentie)...');
    await cdp.screenshot('03_flip_90.png');

    // 04_back.png (fin du flip à 180° avec sceau visible, pause contemplative 280ms)
    await sleep(950);
    console.log('6. Capturing 04_back.png (dos avec sceau à 180°)...');
    await cdp.screenshot('04_back.png');

    // 05_open.png (animation sceau + bascule immédiate vers enveloppe ouverte twin-image)
    await sleep(700);
    console.log('7. Capturing 05_open.png (assemblée ouverte avec poche avant parfaite)...');
    await cdp.screenshot('05_open.png');

    // 06_extraction_mid.png (extraction continue à mi-chemin sans zigzag, 1.5s dans le tween de 3.0s)
    await sleep(1500);
    console.log('8. Capturing 06_extraction_mid.png (extraction continue mi-parcours sans zigzag)...');
    await cdp.screenshot('06_extraction_mid.png');

    // 07_card_ready_html.png (fin de l'extraction 3.0s : carte originale intacte + hotspot + RSVP compact)
    await sleep(2000);
    console.log('9. Capturing 07_card_ready.png (carte originale intacte avec RSVP discret dans espace vide)...');
    await cdp.screenshot('07_card_ready_html.png');

    // Vérification de la présence de la carte originale et du RSVP
    const cardImgSrc = await cdp.eval('document.querySelector(".invitation-card-img")?.src');
    console.log('   Card Image Source:', cardImgSrc);
    ...
    // Clic sur "Valider"
    console.log('11. Clicking "Valider"...');
    await cdp.click('#rsvp-btn-submit');

    // Attente de l'envol de la carte et du message de confirmation
    await sleep(1500);
    ...
    await cdp.click('#envelope-object-3d');
    // Attente du flip (1.85s) + sceau (0.5s) + extraction (3.0s) = ~5.4s
    await sleep(5800);

    // 08_rsvp_mobile.png
    console.log('12. Capturing 08_rsvp_mobile.png (Mobile 390x844 CARD_READY carte originale sans chevauchement)...');
